...
author: "Output by Phenom Web App"
output:
...
graph_function <- function(output, input, x, min_max){
    batch = num_batch
    ...
    gPlot <- ggplot(graph_data, aes(x = Time, y = Mean)) +
      ...
      geom_line() +
      ylim(min_max[1] - 0.1, min_max[2] + 0.1) +
      labs(x = "Time (hours)",
           y = "log(OD)",
           title = title_name) +
      theme_classic()
...
```{r}
get_graph_data <- function(output, input, x){
    batch = num_batch
    every_row <- output[x+1,]
...
output_graphs <- function(output, input){
  ...
  max = -100
  while(y < nrow(output)){
    graph_data <- get_graph_data(output, input, y)
    mean_graph_data <- graph_data[,1]
    
    current_max <- max(unlist(mean_graph_data))
    current_min <- min(unlist(mean_graph_data))
  ...
  while(x < nrow(output)){
    graph_function(output, input, x, min_max)
    x <- x + 1
  }
}
